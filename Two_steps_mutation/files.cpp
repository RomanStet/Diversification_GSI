...
bool readFile(int &Numr, int &Nr,int &pr, int &max_allelesr, int &alleles_initr, double &dpr, double &dgr, double &ar,
                     ...
                     int &NbGenr, double &deltar, int &stepgr, int &sampleSr, double &Unr, int &position_stream, int &stater)
{
	int x;
	bool term;
	do {
	...
	if (x == EOF) {
        ...
        fscanf(fileE,"%d,",&stepgr);  // number of generations at which variables are measured
        fscanf(fileE,"%d,",&sampleSr); // sample size for the measure of inbreeding
